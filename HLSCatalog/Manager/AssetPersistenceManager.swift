...
class AssetPersistenceManager: NSObject {
    ...
    func downloadStream(for asset: Asset) {
        ...
        print("-- Preferred Media Selection: ", preferredMediaSelection.mediaDescription(asset.urlAsset))

        for mediaSelection in asset.urlAsset.allMediaSelections {
            print("-- Possible Additional Media Selection: ", mediaSelection.mediaDescription(asset.urlAsset))
        }
        ...
        guard let task =
            assetDownloadURLSession.aggregateAssetDownloadTask(with: asset.urlAsset,
                                                               ...
                                                               options:
                                                                [AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: asset.stream.minimumDownloadBitrate]) else { return }
...
extension AVMediaSelection {
    func mediaDescription(_ asset: AVURLAsset) -> String {
        var mediaInfo = ""
        if let videoGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.visual) {
            let videoOption = self.selectedMediaOption(in: videoGroup)
            mediaInfo += videoOption.debugDescription + ", "
        }

        if let audioGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.audible) {
            let audioOption = self.selectedMediaOption(in: audioGroup)
            mediaInfo += audioOption.debugDescription + ", "
        }

        if let frameBasedGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.frameBased) {
            let frameBasedOption = self.selectedMediaOption(in: frameBasedGroup)
            mediaInfo += frameBasedOption.debugDescription + ", "
        }

        if let mainGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.isMainProgramContent) {
            let mainOption = self.selectedMediaOption(in: mainGroup)
            mediaInfo += mainOption.debugDescription + ", "
        }

        if #available(iOS 12.0, *) {
            if let originalGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.isOriginalContent) {
                let originalOption = self.selectedMediaOption(in: originalGroup)
                mediaInfo += originalOption.debugDescription + ", "
            }
        }

        if let subtitlesGroup = asset.mediaSelectionGroup(forMediaCharacteristic: AVMediaCharacteristic.legible) {
            let subtitlesOption = self.selectedMediaOption(in: subtitlesGroup)
            mediaInfo += subtitlesOption.debugDescription
        }
